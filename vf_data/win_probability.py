...
class WinProbability:
    ...
    def generate_win_prob_chart_with_single_line(self, round_number, stage, frame_data, frame_num=0, save_to_file = True, p1character="", p2character="", winner_player_number=None):
        last_time_digit = 45
        ...
        # , ha='left'
        ax.set_yticks([0, 1])
        ax.set_yticklabels([f"P2\n{p2character}", f"P1\n{p1character}"], fontsize=12)
        
        if (last_time_digit < 45):
            last_time_digit += 1

        # Set labels and title
        ax.set_title(f'Round {round_number} Win Probability')

        if winner_player_number == 1:
            print(f"p{winner_player_number} winner at {last_time_digit}")
            ax.plot([last_time_digit], [1], 'ro', markersize=12, label="Win!")  # Red dot at the top
            ax.set_title(f'Round {round_number-1} Win Probability')
        elif winner_player_number == 2:
            print(f"p{winner_player_number} winner at {last_time_digit}")
            ax.plot([last_time_digit], [0.1], 'ro', markersize=12, label="Win!")  # Red dot at the bottom
            ax.set_title(f'Round {round_number-1} Win Probability')
